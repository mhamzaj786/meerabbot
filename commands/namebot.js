    ...
    if (custom.length != 0) {
      ...
      //      api.sendMessage(`[Broadcast] The Bot has changed the nickname to "${custom}".`, idThread.threadID)
        ...
        for (const idThread of allThread) {
            ...
            api.changeNickname(`〖  ${(threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "Made by MEERAB KHAN" : global.config.BOTNAME}`, idThread.threadID, idBot, (err) => (err) ? threadError.push(idThread.threadID) : '');
            count+=1;
            await new Promise(resolve => setTimeout(resolve, 500));
        }
